...
func usage() {
	fmt.Printf("GoReplay is a simple http traffic replication tool written in Go. Its main goal is to replay traffic from production servers to staging and dev environments.\nProject page: https://github.com/buger/goreplay\nAuthor: <Leonid Bugaev> leonsbox@gmail.com\nCurrent Version: %s\n\n", VERSION)
	flag.PrintDefaults()
	os.Exit(2)
}
...
func init() {
	...
	flag.Var(&Settings.inputTCP, "input-tcp", "Used for internal communication between GoReplay instances. Example: \n\t# Receive requests from other GoReplay instances on 28020 port, and redirect output to staging\n\tgoreplay --input-tcp :28020 --output-http staging.com")
	...
	flag.Var(&Settings.outputTCP, "output-tcp", "Used for internal communication between GoReplay instances. Example: \n\t# Listen for requests on 80 port and forward them to other GoReplay instance on 28020 port\n\tgoreplay --input-raw :80 --output-tcp replay.local:28020")
	...
	flag.Var(&Settings.inputRAW, "input-raw", "Capture traffic from given port (use RAW sockets and require *sudo* access):\n\t# Capture traffic from 8080 port\n\tgoreplay --input-raw :8080 --output-http staging.com")

	flag.BoolVar(&Settings.inputRAWTrackResponse, "input-raw-track-response", false, "If turned on GoReplay will track responses in addition to requests, and they will be available to middleware and file output.")
	...
	flag.IntVar(&Settings.outputHTTPConfig.workers, "output-http-workers", 0, "GoReplay uses dynamic worker scaling by default.  Enter a number to run a set number of workers.")
	...
	flag.BoolVar(&Settings.outputHTTPConfig.OriginalHost, "http-original-host", false, "Normally GoReplay replaces the Host http header with the host supplied with --output-http.  This option disables that behavior, preserving the original Host header.")
	flag.BoolVar(&Settings.outputHTTPConfig.Debug, "output-http-debug", false, "Enables http debug output.")

	flag.StringVar(&Settings.outputHTTPConfig.elasticSearch, "output-http-elasticsearch", "", "Send request and response stats to ElasticSearch:\n\tgoreplay --input-raw :8080 --output-http staging.com --output-http-elasticsearch 'es_host:api_port/index_name'")

	flag.StringVar(&Settings.outputKafkaConfig.host, "output-kafka-host", "", "Read request and response stats from Kafka:\n\tgoreplay --input-raw :8080 --output-kafka-host '192.168.0.1:9092,192.168.0.2:9092'")
	flag.StringVar(&Settings.outputKafkaConfig.topic, "output-kafka-topic", "", "Read request and response stats from Kafka:\n\tgoreplay --input-raw :8080 --output-kafka-topic 'kafka-log'")
